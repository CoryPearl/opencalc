...
uint8_t Apu2A03::cpuRead(uint16_t addr)
{
    uint8_t data = 0x00;
    if (addr == 0x4015) {
        if (pulse1.len_counter.timer > 0) data |= 0x01;
        if (pulse2.len_counter.timer > 0) data |= 0x02;
        if (triangle.len_counter.timer > 0) data |= 0x04;
        if (noise.len_counter.timer > 0) data |= 0x08;
        if (DMC.memory_reader.remaining_bytes > 0) data |= 0x10;
        if (IRQ) data |= 0x40;
...
void Apu2A03::clockCpuCycles(uint32_t cpu_cycles)
{
#if OPENCALC_GAME_AUDIO_ENABLED
    uint32_t apu_cycles = (cpu_cycles + cpu_half_cycle) / 2;
    cpu_half_cycle = (uint8_t)((cpu_cycles + cpu_half_cycle) & 1u);
    while (apu_cycles-- > 0) {
        clock();
    }
#else
...
inline void Apu2A03::generateSample()
{
    ...
    /* PDM TX accepts signed PCM. Its hardware high-pass filter removes the
     * NES mixer's DC component, so silence remains zero without a pop. */
    audio_buffer[buffer_index] = (int16_t)(sample * 120u);
...
{
    (void)opencalc_audio_write_pcm16(audio_buffer, AUDIO_BUFFER_SIZE, 12);
}
