...
For an MCP client that cannot run `$(...)`, and because the system `python3` may not import a `uv tool` installation, set `command` to the **absolute interpreter path** printed by `cat graphify-out/.graphify_python`:
```json
{
  "mcpServers": {
    "graphify": {
